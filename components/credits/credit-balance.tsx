...
import { ImageIcon, FileTextIcon, Loader2 } from 'lucide-react';
...
export function CreditBalance() {
  ...
  const imageIsLow = data.imagePercentUsed >= 80;
  const textIsLow = data.textPercentUsed >= 80;
    ...
    <div className="px-3 py-2 space-y-3">
      ...
      <div className="space-y-1">
        <div className="flex items-center justify-between text-sm">
          ...
          <span className={`font-medium ${imageIsLow ? 'text-amber-600' : 'text-gray-900 dark:text-gray-100'}`}>
            {data.imageCredits.remaining}/{data.imageCredits.included}
          </span>
        </div>
        <ProgressBar 
          percent={100 - data.imagePercentUsed} 
          isLow={imageIsLow} 
        />
      </div>
      ...
      <div className="space-y-1">
        <div className="flex items-center justify-between text-sm">
          ...
          <span className={`font-medium ${textIsLow ? 'text-amber-600' : 'text-gray-900 dark:text-gray-100'}`}>
            {data.textCredits.remaining}/{data.textCredits.included}
          </span>
        </div>
        <ProgressBar 
          percent={100 - data.textPercentUsed} 
          isLow={textIsLow} 
        />
      </div>
